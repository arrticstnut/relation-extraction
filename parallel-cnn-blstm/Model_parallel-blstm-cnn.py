...
class Model(object):
    ...
    def __init__(self,config,embedding_matrix,is_Training=True):
        ...
        with tf.name_scope("forward"):
            ...
            # --> [batch_size , max_len , nw*num_filters]

            #lstm和cnn的输出进行拼接
            lstm_cnn_feature = tf.concat([lstm_out,cnn_out],axis=2)

            #通过pool层            
            pool_out = self.pool_layer(lstm_cnn_feature)
    ...
    def convolution_layer(self,feature_data):
        ...
        with tf.name_scope("convolution_layer"):
            ...
            nf = feature_data.get_shape().as_list()[2] #n*num_filters_lstm = dim
