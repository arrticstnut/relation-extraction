...
class Model(object):
    ...
    def lstm_layer(self,input_feature):
            ...
            with tf.variable_scope("Bi-lstm_layer"):
                cell = rnn.LSTMCell(self.num_filters, use_peepholes=True,
                                       initializer=self.initializer_layer, state_is_tuple=True)
                output, output_state = tf.nn.dynamic_rnn(cell=cell, dtype=tf.float32, inputs=input_feature)

                #outout为BLSTM的输出， 是一个元组，为（前向output，后向output)
                #out_lstm将前后想的BLSTM的输出拼接在一起
                out_lstm = tf.concat(output, axis=2) #[batch_size , max_len , n*self.num_filters]
