...
def load_w2v_embedding(emd_wiki_w2v_path,word_dict):
    num_word = len(word_dict)+1

    #打开词向量文件
    h_emd_file = open(emd_wiki_w2v_path,'r',encoding='utf-8')

    #第一行保存了向量的总行数和维度
    vec_dim = int(h_emd_file.readline().split()[1])

    embedding = np.random.uniform(-0.1,0.1,size=(num_word,vec_dim))
    emd_vacb = {}

    #从第2行开始读取
    for line in h_emd_file:
        line = line.split()
        emd_vacb[line[0].strip().lower()] = line[1:]

    for word in word_dict:
        if word in emd_vacb:
            #embedding[word_dict[word]] = [val for val in vec_lines[emd_vacb[word]].split()]
            embedding[word_dict[word]] = emd_vacb[word]
    embedding[0] = np.zeros(vec_dim)
...
def dataset2id(dataset,word_dict,max_length):
    ...
    for sent, p1, p2 in zip(sentences_id, e1_pos, e2_pos):
        ...
        dist_e1.append([pos_embed(p1[1] - idx,max_length) for idx, _ in enumerate(sent)])
        dist_e2.append([pos_embed(p2[1] - idx, max_length) for idx, _ in enumerate(sent)])

    return sentences_id,e1_vec,e2_vec,dist_e1,dist_e2,relation

def pos_embed(x,max_length):

    ##<=======================================================>
    #将区间[-(max_length-1),max_length-1]平移到区间[1,2*max_length-1]
    return (x + max_length)
    ##</=======================================================>
    '''
    if x < -60:
