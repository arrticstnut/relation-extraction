...
class Model(object):
    ...
    def __init__(self,config,embedding_matrix,is_Training=True):
        ...
        with tf.name_scope("forward"):
            ...
            #通过cnn层
            cnn_out = self.convolution_layer(input_feature)

            #通过lstm层
            lstm_out = self.lstm_layer(cnn_out)
            # --> [batch_size , max_len , 2*self.num_filters]
            #if is_Training:
                #lstm_out = tf.nn.dropout(lstm_out, self.dropout)

            #通过pool层            
            pool_out = self.pool_layer(lstm_out)
    ...
    def convolution_layer(self,feature_data):
        ...
        with tf.name_scope("convolution_layer"):
            ...
            feature_data = tf.reshape(feature_data,[-1, self.max_len,self.input_feature_dim,1])
            ...
            for filter_size in window_heights:
                with tf.variable_scope('conv-%s' % filter_size):
                    #卷积核
                    cnn_w = tf.get_variable(shape=[filter_size,w_windows,1,num_filters_cnn],
                    ...
                    conv = tf.nn.conv2d(feature_data,cnn_w,strides=[1,1,self.input_feature_dim,1],padding="SAME")
